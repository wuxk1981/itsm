...
import com.zdhy.platform.itsm.core.utils.TreeObject;
import com.zdhy.platform.itsm.core.utils.TreeUtil;
import com.zdhy.platform.itsm.core.utils.Common;
import com.zdhy.platform.itsm.core.utils.PropertiesUtils;
import com.zdhy.platform.itsm.entity.Params;
		...
		try {
			...
			for (int i = 0, inv = id.length; i < inv; i++) {
				if (!Common.isEmpty(id[i])) {
					resourcesService.deleteByPrimaryKey(id[i]);
				}
			}
			map.put("flag", "true");
		} catch (Exception e) {
