...
//	}
	 ...
	 * @throws Exception 
	 * 
	 * http://localhost:8082/itsm/background/dic/add?dickey=sex&dicname=SEX&description=SEXDEC
	 * 
	 */
	@RequestMapping("add")
	@ResponseBody
	...
	public String editUI(Model model,String dicId) {
		Dic dic = dicService.selectByPrimaryKey(dicId);
		...
		try {
			...
			for (String string : id) {
				if(!Common.isEmpty(string)){
					dicService.deleteByPrimaryKey(string);
				}
			}
			map.put("flag", "true");
		} catch (Exception e) {
...
//	}
	...
	public Map<String, Object> update(Model model, Dic dic) {
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			dicService.updateByPrimaryKey(dic);
			map.put("flag", "true");
		} catch (Exception e) {
			map.put("flag", "false");
		}
